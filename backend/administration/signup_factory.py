...
class SignUpFactory:
    ...
    def __init__(self, request):
        ...
        self.profile_id = self.data.get("profile_id") if self.data.get("profile_id") else str(uuid.uuid4())
        self.request.user.profile_id = self.profile_id
        print("self.request.uer.profile_id", self.profile_id)
        self.favourites_playlist_id  = str(uuid.uuid4())

    def create(self):

        profile = self.create_profile()
        print("profile profile id", profile.profile_id)
    ...
    def create_profile(self):
        ...
        profile = Profiles.objects.create(
            ...
            profile_name = self.data.get("fullname") if isinstance(self.data.get("fullname"), str) else None,
            profile_handle = self.data.get("username"),
            profile_superuser_list = [self.profile_id],
